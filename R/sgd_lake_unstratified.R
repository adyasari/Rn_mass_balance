...
Rn_ss_calc <- function(in_tbl, optim_driver = NULL){
  ...
  dat_tbl <- in_tbl %>%
    mutate(
      ...
      Rn_ss__dpmm3 = if (is.null(optim_driver)) mean(Rn_wat__Bqm3, na.rm = TRUE) else optim_driver, # driver of optimization
      
      Rn_ini_inv__dpmm2 = depth__m * Rn_ss__dpmm3,
      Rn_ss_flux__dpmm2hr = lambda__hr * Rn_ini_inv__dpmm2, 
      Rn_ss_inv__dpmm2 = Rn_ss_flux__dpmm2hr / lambda__hr,
      mc_Rn_water__dpmm3 = c(Rn_wat__Bqm3[2], rep(NA_real_, length(time) - 1)), # if first row deleted then start from 1 rather than 2
    )
  
  for(t_ind in seq_along(dat_tbl$time)[-1]) {
    dat_tbl$mc_Rn_water__dpmm3[t_ind] = dat_tbl %$% {mc_Rn_water__dpmm3[t_ind - 1] * exp(-lambda__hr[t_ind] * meas_t__h[t_ind]) + (Rn_ss_flux__dpmm2hr[t_ind] - (mc_Rn_water__dpmm3[t_ind-1] - (kw_air[t_ind] * Rn_air__Bqm3[t_ind])) * k_piston_Rn__mmin[t_ind] * 60) * (1 - exp(-lambda__hr[t_ind] * meas_t__h[t_ind])) / lambda__hr[t_ind] / depth__m[t_ind]}
  }
  ...
  dat_tbl <- dat_tbl %>%
    mutate(
      ...
      m_inv_Rn_decay_corr__dpmm2 = c((lag(mc_Rn_water__dpmm3) * exp(-lambda__hr * meas_t__h) * depth__m)[1:2], rep(NA_real_, length(time) - 2)),  # if first row deleted then start from 1 rather than 2
      m_SGD_inv_Rn__dpmm2 = Rn_ss_flux__dpmm2hr * (1 - exp(-lambda__hr * meas_t__h)) / lambda__hr,
  ...
  dat_tbl <- dat_tbl %>%
    mutate(
      ...
      sq_err = (final_mc_Rn__dpmm3 - Rn_wat__Bqm3)^2,
...
optim_start <- dat_tbl %>% 
  summarize(mean(Rn_ss__dpmm3, na.rm = TRUE)) %>% 
  pull()
